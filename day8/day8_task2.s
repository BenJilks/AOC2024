...
.loop_start:
    cmp rdx, 0
    jl .stop
    cmp rdx, qword [g_width]
    jge .stop
    cmp rcx, 0
    jl .stop
    cmp rcx, qword [g_height]
    jge .stop
...
compute_antinodes_for_pair:
    ...
    ; dx = node_a_x - node_b_x
    ; dy = node_a_y - node_b_y
    ; start_x = node_a_x
    ; start_y = node_a_y
    ...
    mov rdx, qword [node_a_x]
    mov rcx, qword [node_a_y]
    call follow_antinodes

    ; dx = node_b_x - node_a_x
    ; dy = node_b_y - node_a_y
    ; start_x = node_b_x
    ; start_y = node_b_y
